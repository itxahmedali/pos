...
export class MenuItemsComponent {
  ...
  constructor(private cd: ChangeDetectorRef, private route: Router) {
    route.events.subscribe((val: any) => {
      const url = val?.url?.split('/')[2]
      if (url) {
        this.Menus.map((e: any) => {
          const lowercaseObj = Object.keys(e).reduce(
            (obj: any, key: string) => {
          ...
          if (lowercaseObj.hasOwnProperty(url.toLowerCase())) {
            this.MenuSelected = lowercaseObj[url.toLowerCase()];
          }
        });
      }
    });
